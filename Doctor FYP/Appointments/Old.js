  ...
  const currentid=localStorage.getItem('doctorid');
    ...
    const getdata=async()=>{
        const response = await axios.get(
            "http://localhost:3010/appointment/pastappointments/"+currentid
        );
    ...
    const handleFilter=(e)=>{
        if(e.target.value==''){
            ...
            const filterData=searchApi.filter(item=>item.date.toLowerCase().includes(e.target.value.toLowerCase()));
            setUsers(filterData);
        }
